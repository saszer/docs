...
You can use API keys/tokens in two ways:

**Method 1: X-API-Key Header (Recommended for MCP)**
...
<Note>
**Token Types:** Both JWT tokens (from OAuth login) and API tokens (from `/api/admin/api-keys`) can be used in the `Authorization: Bearer` header. Use whichever authentication method fits your use case.
</Note>
